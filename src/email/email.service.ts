...
export class EmailService {
  ...
  async sendFriendRequestEmail(to: string, recipientName: string, senderName: string): Promise<EmailResult> {
    const html = `
      <!DOCTYPE html>
      <html>
      <head>
      ...
      <body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; background-color: #0f0f23; color: #f1f1f1; padding: 40px 20px; margin: 0;">
        <div style="max-width: 600px; margin: 0 auto; background-color: #1a1a2e; border-radius: 12px; overflow: hidden;">
          ...
          <div style="padding: 40px 30px;">
            ...
            <div style="text-align: center; margin: 30px 0;">
              <a href="${this.frontendUrl}/friends" 
  ...
  async sendFriendAcceptedEmail(to: string, recipientName: string, friendName: string): Promise<EmailResult> {
    const html = `
      <!DOCTYPE html>
      <html>
      <head>
